...
function OrderById() {
  ...
  const seoData = useSEO({
    title: (() => {
      if (loading) return "Cargando pedido - Chroma";
      if (error) return "Error al cargar pedido - Chroma";
      if (!order) return "Pedido no encontrado - Chroma";
      return `Pedido #${order.number} - ${order.status} - Chroma`;
    })(),

    description: (() => {
      if (loading) return "Cargando detalles del pedido en Chroma";
      if (error)
        return "Error al cargar el pedido. Intenta nuevamente o contacta soporte.";
      if (!order)
        return "El pedido solicitado no fue encontrado en nuestro sistema.";

      const itemCount = order.items.length;
      const itemText = itemCount === 1 ? "producto" : "productos";
      return `Pedido #${order.number} con ${itemCount} ${itemText}. Estado: ${order.status}. Total: ${order.total}`;
    })(),

    path: `/orders/detail/${orderid}`,
    image: "/img/logo-chroma-ecommerce.png",
    keywords: `pedido ${orderid}, orden Chroma, estado pedido, seguimiento compra, detalle pedido`,
    type: "website",
    noIndex: true,
  });
